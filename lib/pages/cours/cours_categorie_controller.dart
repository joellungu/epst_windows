...
class CoursCategorieController extends GetxController with StateMixin<List> {
  ...
  getAllClasse(String idClasse, String typeFormation) async {
    //categorie = categorie.toLowerCase();
    //Response rep = await requette.getE("coure/all");
    change([], status: RxStatus.loading());
    //
    print("idClasse: $idClasse, typeFormation: $typeFormation");
    http.Response rep = await http.get(
      Uri.parse(
        "${Connexion.lien}cours/allcours?idClasse=$idClasse&typeFormation=$typeFormation",
      ),
      headers: {
        "Accept": "*/*",
...
//
  deleteCours(Map cours) async {
    ...
    if (rep.statusCode == 200 ||
      ...
      getAllClasse(cours['cls'], cours['propriete']);
      //change(listeDeClasse, status: RxStatus.success());
    } else {
      print("rep: ${rep.body}");
      print("rep: ${rep.statusCode}");
